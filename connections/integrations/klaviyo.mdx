...
5. Click on **Create Private API Key**, give your key a name and choose **Full Access Key** and press **Create**.

6. Copy the newly created **Private API Key**, you will need it later.
   &nbsp;

7. In your Converge dashboard, under Connections, click <IconText name="circle-plus" text="Create New Connection" iconLocation="before" iconType="regular"/ > and fill in your **Public API Key** in the **Public Token** field and your **Private API Key** in the **API Key**-field. 
   &nbsp;

8. Click on the **Advanced**-tab to set up your events.
    <Frame>
   ![klaviyo-2](/images/connections/klaviyo/klaviyo-install-1.png)
   </Frame>

9. Choose the events for which you want to set up the Converge Connection. 
    <Note>
      If you are using any of Klaviyo's prebuilt integrations, then you will want to make sure to **not** send `Started Checkout` and `Placed Order`-events. Otherwise, you will want to send all events.
    </Note>
...
A common usecase of the Klaviyo Destination is to build Klaviyo Email Flows based on Converge Events. 
...
<Accordion title='Extending your Converge Event implementation for Klaviyo'>
...
<Accordion title="I already have the Klaviyo integration for Shopify, should I also set up a Converge connection?">
  If you are using any of Klaviyo's prebuilt integrations, then there is still value in setting up the Converg Klaviyo Connection to track the `Viewed Product`, `Viewed Category`, and `Added To Cart`-events or to send [Custom Events](/sources/extending-the-spec/custom-events.mdx) to Klaviyo.
</Accordion>
</AccordionGroup>
