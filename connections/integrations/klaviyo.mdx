...
<Accordion title="I already have the Klaviyo integration for Shopify, should I also set up a Converge connection?">
  If you are using any of Klaviyo's prebuilt integrations, then there is still value in setting up the Converge Klaviyo Connection to track the `Viewed Product`, `Viewed Category`, and `Added To Cart`-events or to send [Custom Events](/sources/extending-the-spec/custom-events.mdx) to Klaviyo.
</Accordion>
</AccordionGroup>
