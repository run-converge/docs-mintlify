...
The steps for setting up these custom properties are functionally similar to the steps described above in the [Custom Event Properties](#custom-event-properties)-paragraph.
The only distinction is that you should choose an `user-scoped custom dimension` in the GA4 <IconText name="shapes" text="Custom definitions" iconType="regular"/> settings.
...
#### Pass consent parameters on serverside events
...
Includes the customer ID as the `user_id` for GA4 events. 
The User Id feature in GA4 has some known issues, please only turn this on if you have a good reason to do so.
