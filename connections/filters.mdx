...
Subscription brands typically want to filter out recurring purchases when forwarding to ad platforms. To achieve this, you can set up a Filter on the `Placed Order` event on a per-connection basis. 
...
You might want to set up different Google Ads Connections for each store for example. 
...
<Frame>
![CAD Example](/images/connections/filters/multi-store-example.png)
</Frame>
