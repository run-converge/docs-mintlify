...
<Accordion title="What is happening with Consent Mode v2?" icon="lightbulb" iconType="duotone">
  ...
  - Implementing consent management reduces the amount of data that can be collected by GA4 and Google Ads. Google will try to fill the gap by modelling missing data. This will lead to less accurate reporting. 
</Accordion>

<Accordion title="How will this impact my Google Ads Performance and Google Analytics Data?">
  As of early March, it is still unclear how and whether data without consent will be processed.
</Accordion>

<Accordion title="How can I check whether I am ready for Consent Mode v2?">
<Accordion title="Is Converge compatible with Consent Mode v2?">
Yes, the Converge connections are compatible with any consent implementation, either through a CMP platform, a custom implementation, or default setup.
</Accordion>

</Accordion>

<Accordion title="I am not ready for Consent Mode v2, what should I do?">
</Accordion>
...
<AccordionGroup>
    <Accordion title="A Consent event is fired">
        In the Summary pane, look for the `Consent` events. These events indicate that consent state is communicated to the Google tags.
    </Accordion>
...
No data is collected for users for which consent was denied, whether by default or through a explicit deny.

### Google Analytics 4
In GA4, no data is reported for visitors who denied the `analytics_storage` consent.
By default, Google does attempt to bridge this gap through behioural modeling.
...
### Google Ads
The same holds for Google Ads. Google cannot directly attribute conversions when consent was denied.
Again, Google applies conversion modelling to try to compensate for the signal loss.
The conversions reported in Google Ads include these [modelled conversions](https://support.google.com/google-ads/answer/10548233?hl=en-GB).

Moreover, due to the Google consent V2 change, when the `ad_personalization` parameter was not granted, they cannot use the conversion for dynamic remarketing.
