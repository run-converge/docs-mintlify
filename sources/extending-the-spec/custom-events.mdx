...
Although Converge will automatically track the essential events in your Website, there are sometimes user actions that you may want to track that are not tracked by default.
...
- <OutboundLink  linkText="Client-side custom events with Converge JS" linkTarget="https://learn.runconverge.com/sources/website-integrations/converge-pixel#manually-instrumenting-events" /> or <Tooltip tip="Client-side sources are generally wrappers around the Converge JS pixel"> other client-side sources </Tooltip>.
- <OutboundLink  linkText="Server-side custom events with Server Webhook" linkTarget="https://learn.runconverge.com/sources/server-integrations/webhook" />
