...
Although Converge will automatically track the essential events in your Website, there are sometimes user actions that you may want to track that are not tracked by default. Custom events allow you to track arbitrary events based on user actions with code changes.
...
In addition to common numeric (e.g. **lower than**, **greater than or equal to**, etc) and string operators (**contains**, **starts with**, etc), some condition variables also support some more advanced operators: 
