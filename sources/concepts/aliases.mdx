...
The exact aliases you choose depends on your specific customer journey, commonly used aliases are:
...
There are two main guidelines for setting up aliases:
...
```mermaid
flowchart LR
  id1("\`*Browser* \n Viewed Page\`") --> id2("\`*Browser* \n Added To Cart \`") --> id3("\`*Browser* \n Started Checkout \`") --> id4("\`*Browser* \n Added Payment Info \`") --> id5("\`*Server* \n Placed Order \`")
```
...
```mermaid
flowchart LR
  id1("\`*Browser* \n Viewed Page\`") --> id2("\`*Browser* \n Added To Cart \n **alias=cart_token_xyz** \`") --> id3("\`*Browser* \n Started Checkout \n **alias=checkout_token_123**  \`") --> id4("\`*Browser* \n Added Payment Info \n **alias=john@smith.com**  \`") --> id5("\`*Server* \n Placed Order \n **alias=cart_token_xyz** \n **alias=checkout_token_123** \n **alias=john@smith.com** \`")
```
...
2. There are stitching identifiers available early in the customer journey, making sure that a client-side events with relevant aliases will arrive before the server-side `Placed Order`-event.
