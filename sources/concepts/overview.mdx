...
-  <OutboundLink linkText="Profile properties" linkTarget="https://learn.runconverge.com/sources/concepts/profile-properties"/>: to include browser-side information into the server-side order-event.
- <OutboundLink linkText="Deduplication" linkTarget="https://learn.runconverge.com/sources/concepts/deduplication"/>: to make sure that the same order does not get processed twice.
- <OutboundLink linkText="Aliases" linkTarget="https://learn.runconverge.com/sources/concepts/aliases"/>: to recognize which browser-side events and which server-side events come from the same user.