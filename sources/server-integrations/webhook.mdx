...
<AccordionGroup>
    ...
    <Accordion title="Setting up the Converge Webhook Source" icon="server" >
  ...
  <Steps>
    ...
    <Step title="Send a dummy event">
        4. Click on your created newly created Source and copy the webhook endpoint.
       
        5. Verify that your Webhook Source is working by sending a dummy [Placed Order](../converge-spec#placed-order)-event request. 
...
If the pre-built server integrations does not cover the the applicable conversions from the [Converge Event Spec](../converge-spec); or if you want to add custom events from your website then you will need to manually instrument these using the Converge Pixel.
...
- `event_id`: The ID of the event, used for [deduplication](../source-concepts#deduplication)
- `properties`: The event properties you want to pass, covering the Converge Spec and possible your own custom properties.
- `profile_properties`: The [profile properties](../source-concepts#profile-properties)
- `aliases`: Any [aliurases](../source-concepts#aliases)
...
<Note> You should typically implement [Placed Order](../converge-spec#placed-order) and [Started Subscription](../converge-spec#started-subscription) events server-side. You can find a full list in the [Converge Spec here](../converge-spec) </Note>