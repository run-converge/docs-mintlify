...
## Installation Instructions
  ...
  <Steps>
    ...
    <Step title="Send a dummy event">
        ...
        <Note>
            The addition of the Webhook Token is a part of recent updates aimed at improving the custom webhooks integration experience. This change does not affect any existing webhook integrations. These updates are specifically designed to enhance the visibility of error messages, making it easier for you to diagnose and resolve any issues. You can continue to use your existing integration if you prefer.
        </Note>
        ...
        5. Verify that your Webhook Source is working by sending a dummy [Placed Order](/sources/converge-spec#placed-order) event request. Ensure the request meets the following parameters outlined in [Manually instrumenting events](/sources/server-integrations/webhook#manually-instrumenting-events)
         ...
         <Note> You should fill in your own webhook endpoint and webhook token from the Converge UI in the code example below</Note>
   <CodeGroup>
...
| `aliases` | `array[string]` | **Required**. Any [aliases](/sources/source-concepts#aliases) |
| `timestamp` | `timestamp` | **Optional**. The date and time when the event occurred, expressed as a UTC timestamp in ISO 8601 format (e.g., 2024-09-02T12:34:56Z). If not provided, the current UTC time will be used by default. This timestamp should be a string representing the date and time in UTC to ensure consistency across different systems.|
