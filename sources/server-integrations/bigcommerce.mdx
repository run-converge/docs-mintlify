...
<AccordionGroup>
    <Accordion title="Retrieve your Store-Level API Token" icon="key">
    <Steps>
    ...
    <Step title="Configure your API Token">
        ...
        6. Click **Save**. You will be prompted to download a `.txt`-file containing credentials you will need to set up your Converge source.
    </Step>

</Steps>
   </Accordion >

    <Accordion title="Connect Converge BigCommerce Server integration" icon="plug" >
  ...
  <Steps>
    ...
    <Step title="Connect the Source to BigCommerce">
        5. Enter the hash parameter from the API path retrieved in Step 4 above.
            <Info> If your API path is `https://api.bigcommerce.com/stores/xxxxxx/`, the hash is `xxxxxx`. </Info>
        6. Paste Access Token from the `.txt`-file in Step 6 above in the **Access Token** field.
        7. Unless you have a good reason not to, you should leave **Storefronts** and **Start Date** blank. Click on **Create Source**.
