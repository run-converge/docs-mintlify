...
Converge supports cross-domain tracking by means of the methods below. We can stich sessions across different domains and can make sure that cookie information, UTM attribution, etc. will work correctly.
...
Converge allows for cross-domain tracking by means of the `__cvg_uuid` parameter and cookie. 
...
If there is a `__cvg_uid` set in the URL as a hyperparam, the cookie will be set to this value.


```mermaid
flowchart LR
  id1("\`*User* visits **myfirstdomain.com** and gets assigned **__cvg_uid=1234**\`") -. "Clicks on link myotherdomain.com/?__cvg_uid=1234" .-> id2("\`*User* visits **myotherdomain.com** and the Converge reads the id from the URL\`") ~~~ id3(["\`**Session stiched**  ✓ \`"])
...
You will need to manually instrument adding the `?__cvg_uid` URL parameter and its corresponding cookie value for each outbound link in that case.
</Warning>
