...
```mermaid
flowchart LR
  id1("\`*User* visits **myfirstdomain.com** and gets assigned **__cvg_uid=1234**\`") -. "Clicks on link myotherdomain.com/?__cvg_uid=1234" .-> id2("\`*User* visits **myotherdomain.com** and the Converge reads the id from the URL\`") ~~~ id3(["\`**Session stitched**  ✓ \`"])
